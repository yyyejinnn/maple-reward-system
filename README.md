...
- JwtGuad, RoleGuard을 전역으로 설정하여 보안을 높이고, 로그인 같은 공개 API에 대해서만 Public 데코레이터로 접근을 허용했습니다.
- 접근 권한이 매우 제한적인 감사자(AUDITOR)는 기본적으로 모든 요청을 차단 하되, 허용된 API에 대한 접근 권한을 주는 whitelist 방식을 적용했습니다.
